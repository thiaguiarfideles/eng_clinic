...
ALLOWED_EXTENSIONS = {'jpg', 'jpeg', 'png', 'gif', 'csv'}  # Extensões de arquivo permitidas
...
df = pd.read_csv('C:/Users/Default/Desktop/MATERIAIS5.csv', delimiter=';')
    ...
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                flash('Arquivo inválido. Certifique-se de selecionar um arquivo CSV.', 'danger')
...
# Função para verificar se o arquivo é um CSV permitido
def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

# Função para processar o arquivo CSV e preencher a tabela de materiais
def process_csv_file(file_path):
    df = pd.read_csv(file_path)

    for index, row in df.iterrows():
        tipo_material = row.get('tipo_material', row.get(' tipo_material'))
        material = row.get('material', row.get('material '))
        descricao = row.get('descricao', row.get('descricao'))
        unidade_medida = row.get('unidade_medida', row.get('unidade_medida'))
        quantidade = row.get('quantidade', row.get('quantidade'))
